...
// dictionary into their bundle — re-exported here so every existing
// `from "@/lib/i18n"` import keeps working unchanged.
export { SUPPORTED_LANGS, DEFAULT_LANG, TRUST_SOURCES, SITE } from "./site";
...
export const T = {
  // -- /[lang]/social/ -- the venues a visitor is plausibly looking at when
  // they arrive from Instagram or TikTok, split by how much independent
  // evidence backs each one. See lib/social.ts for why it isn't a
  // "trending" page.
  soc_nav: { en: "Seen on social", ko: "소셜에서 본 곳", th: "เห็นจากโซเชียล", zh: "社交平台所见", ja: "SNSで見た店", id: "Terlihat di sosial", vi: "Thấy trên mạng xã hội" },
  soc_h1: {
    en: "The Thai wellness spots you'll see on Instagram and TikTok",
    ko: "인스타그램·틱톡에서 보게 되는 태국 웰니스 업체들",
    th: "สถานที่เวลเนสในไทยที่คุณจะเห็นบน Instagram และ TikTok",
    zh: "你会在 Instagram 和 TikTok 上看到的泰国康养场所",
    ja: "InstagramやTikTokで見かけるタイのウェルネス施設",
    id: "Tempat wellness Thailand yang akan Anda lihat di Instagram dan TikTok",
    vi: "Những địa điểm wellness Thái Lan bạn sẽ thấy trên Instagram và TikTok",
  },
  soc_sub: {
    en: "{total} venues in our index run an Instagram or TikTok account. A polished feed tells you nothing about whether a place is real, so we split them by what independent evidence actually exists.",
    ko: "우리 인덱스의 {total}곳이 인스타그램 또는 틱톡 계정을 운영합니다. 잘 꾸민 피드는 그 업체가 진짜인지 알려주지 않기 때문에, 독립적인 근거가 실제로 얼마나 있는지로 나눴습니다.",
    th: "{total} แห่งในดัชนีของเรามีบัญชี Instagram หรือ TikTok ฟีดที่สวยงามไม่ได้บอกว่าที่นั่นมีจริงหรือไม่ เราจึงแยกตามหลักฐานอิสระที่มีจริง",
    zh: "我们索引中有 {total} 家场所运营 Instagram 或 TikTok 账号。精美的动态无法说明一家店是否真实，因此我们按实际存在的独立证据来划分。",
    ja: "インデックス内の{total}件がInstagramまたはTikTokを運用しています。整ったフィードは実在の証明にならないため、独立した裏付けの量で分けました。",
    id: "{total} tempat di indeks kami punya akun Instagram atau TikTok. Feed yang rapi tidak membuktikan apa pun, jadi kami memilahnya berdasarkan bukti independen yang benar-benar ada.",
    vi: "{total} địa điểm trong chỉ mục của chúng tôi có tài khoản Instagram hoặc TikTok. Một feed đẹp không chứng minh điều gì, nên chúng tôi phân loại theo bằng chứng độc lập thực có.",
  },
  soc_thin_h2: {
    en: "Thin evidence — worth a second look ({count})",
    ko: "근거가 얇음 — 한 번 더 확인할 것 ({count}곳)",
    th: "หลักฐานน้อย — ควรตรวจสอบเพิ่ม ({count})",
    zh: "证据薄弱 — 值得再核实（{count} 家）",
    ja: "裏付けが薄い — 要確認（{count}件）",
    id: "Bukti tipis — perlu dicek lagi ({count})",
    vi: "Bằng chứng mỏng — nên kiểm tra thêm ({count})",
  },
  soc_thin_sub: {
    en: "A social account we found, but at most 3 of our 9 source checks and fewer than 30 Google reviews. This is not an accusation — plenty of them are new, or simply small. It means we could not confirm much, and you should look harder before you pay.",
    ko: "소셜 계정은 찾았지만, 9개 출처 검사 중 최대 3개만 통과했고 구글 리뷰도 30개 미만입니다. 비난이 아닙니다 — 신규이거나 그냥 작은 곳도 많습니다. 다만 우리가 확인할 수 있는 게 적었다는 뜻이니, 결제 전에 더 알아보세요.",
    th: "เราพบบัญชีโซเชียล แต่ผ่านการตรวจสอบเพียง 3 จาก 9 แหล่ง และมีรีวิว Google น้อยกว่า 30 รายการ นี่ไม่ใช่การกล่าวหา หลายแห่งเพิ่งเปิดหรือเป็นร้านเล็ก แต่หมายความว่าเรายืนยันได้ไม่มาก",
    zh: "我们找到了社交账号，但九项来源核查中最多只通过三项，且 Google 评论少于 30 条。这不是指控 — 其中许多是新店或规模较小。但这意味着我们能证实的有限，付款前请多加了解。",
    ja: "SNSアカウントは見つかりましたが、9つの照合のうち最大3つ、Googleレビューも30件未満です。非難ではありません — 新規店や小規模店も多くあります。ただ確認できたことが少ないので、支払う前に調べてください。",
    id: "Kami menemukan akun sosialnya, tetapi hanya lolos maksimal 3 dari 9 pemeriksaan sumber dan ulasan Google kurang dari 30. Ini bukan tuduhan — banyak yang baru atau kecil. Artinya kami belum bisa memastikan banyak hal.",
    vi: "Chúng tôi tìm thấy tài khoản mạng xã hội, nhưng chỉ đạt tối đa 3 trong 9 kiểm tra nguồn và dưới 30 đánh giá Google. Đây không phải cáo buộc — nhiều nơi mới mở hoặc nhỏ. Nghĩa là chúng tôi chưa xác nhận được nhiều.",
  },
  soc_ok_h2: {
    en: "Corroborated ({count})",
    ko: "교차 검증됨 ({count}곳)",
    th: "ยืนยันแล้ว ({count})",
    zh: "已获佐证（{count} 家）",
    ja: "裏付けあり（{count}件）",
    id: "Terkonfirmasi ({count})",
    vi: "Đã đối chiếu ({count})",
  },
  soc_ok_sub: {
    en: "A social account plus independent evidence: Google review depth, photos, video coverage, community threads, domain history. Ranked by trust score.",
    ko: "소셜 계정에 더해 독립적인 근거가 있는 곳: 구글 리뷰 깊이, 사진, 영상 노출, 커뮤니티 스레드, 도메인 이력. 트러스트 스코어 순.",
    th: "มีบัญชีโซเชียลพร้อมหลักฐานอิสระ: ความลึกของรีวิว Google รูปภาพ วิดีโอ กระทู้ชุมชน ประวัติโดเมน",
    zh: "既有社交账号，也有独立证据：Google 评论深度、照片、视频报道、社区讨论、域名历史。按信任分排序。",
    ja: "SNSアカウントに加え独立した裏付けがある施設：Googleレビューの厚み、写真、動画、コミュニティ投稿、ドメイン履歴。信頼スコア順。",
    id: "Punya akun sosial plus bukti independen: kedalaman ulasan Google, foto, liputan video, utas komunitas, riwayat domain.",
    vi: "Có tài khoản mạng xã hội cùng bằng chứng độc lập: chiều sâu đánh giá Google, ảnh, video, thảo luận cộng đồng, lịch sử tên miền.",
  },
  soc_sources_short: {
    en: "{n}/9 sources", ko: "출처 {n}/9", th: "{n}/9 แหล่ง", zh: "{n}/9 来源",
    ja: "{n}/9 情報源", id: "{n}/9 sumber", vi: "{n}/9 nguồn",
  },
  soc_meta_title: {
    en: "Seen on Instagram or TikTok? Thai wellness venues, evidence-ranked",
    ko: "인스타·틱톡에서 봤나요? 태국 웰니스 업체 근거별 정리",
    th: "เห็นบน Instagram หรือ TikTok? สถานที่เวลเนสในไทย จัดอันดับตามหลักฐาน",
    zh: "在 Instagram 或 TikTok 上看到过？泰国康养场所，按证据排序",
    ja: "InstagramやTikTokで見た？タイのウェルネス施設を裏付け順に",
    id: "Terlihat di Instagram atau TikTok? Tempat wellness Thailand, diurutkan bukti",
    vi: "Thấy trên Instagram hoặc TikTok? Địa điểm wellness Thái Lan, xếp theo bằng chứng",
  },
  // -- /[lang]/w/[collection]/ -- searches our seven niches hide (sauna,
  // pilates, ice bath...). Templated: only the collection name varies, so
  // lib/collections.ts carries the per-locale names and these carry the copy.
  coll_h1: {
    en: "{name} in Thailand, verified",
    ko: "검증된 태국 {name}",
    th: "{name} ในไทย ตรวจสอบแล้ว",
    zh: "泰国{name}，已核实",
    ja: "検証済みのタイの{name}",
    id: "{name} di Thailand, terverifikasi",
    vi: "{name} tại Thái Lan, đã xác minh",
  },
  coll_sub: {
    en: "{count} venues, each cross-checked against {sources} independent sources. Ranked by trust score — nobody can pay to move up.",
    ko: "{count}곳 · 각각 {sources}개 독립 소스로 교차 검증. 트러스트 스코어 순 — 돈으로 순위를 올릴 수 없습니다.",
    th: "{count} แห่ง ตรวจสอบข้ามจาก {sources} แหล่งอิสระ เรียงตามคะแนนความน่าเชื่อถือ",
    zh: "{count} 家场所，均经 {sources} 个独立来源交叉核实。按信任分排序，无法付费提升。",
    ja: "{count}件、それぞれ{sources}件の独立した情報源で照合。信頼スコア順、課金で順位は動きません。",
    id: "{count} tempat, masing-masing diperiksa silang dengan {sources} sumber independen. Diurutkan berdasarkan skor kepercayaan.",
    vi: "{count} địa điểm, mỗi nơi được đối chiếu với {sources} nguồn độc lập. Xếp theo điểm tin cậy.",
  },
  coll_meta_title: {
    en: "{name} in Thailand — {count} verified venues",
    ko: "태국 {name} — 검증된 {count}곳",
    th: "{name} ในไทย — {count} แห่งที่ตรวจสอบแล้ว",
    zh: "泰国{name} — {count} 家已核实场所",
    ja: "タイの{name} — 検証済み{count}件",
    id: "{name} di Thailand — {count} tempat terverifikasi",
    vi: "{name} tại Thái Lan — {count} địa điểm đã xác minh",
  },
  coll_meta_desc: {
    en: "Every {name} venue in Thailand we could independently verify — {count} of them, cross-checked against Google, Reddit, Pantip, Naver, YouTube and domain history. No paid placement. Saw one on Instagram or TikTok? Check it here.",
    ko: "태국에서 독립적으로 검증할 수 있었던 모든 {name} — {count}곳. 구글·레딧·판팁·네이버·유튜브·도메인 이력으로 교차 검증. 광고 순위 없음. 인스타·틱톡에서 봤다면 여기서 확인하세요.",
    th: "ทุก {name} ในไทยที่เราตรวจสอบได้ — {count} แห่ง ตรวจสอบข้ามจาก Google, Reddit, Pantip, Naver, YouTube และประวัติโดเมน",
    zh: "我们能独立核实的全部泰国{name} — 共 {count} 家，交叉验证自 Google、Reddit、Pantip、Naver、YouTube 与域名历史。无付费排名。",
    ja: "独立して検証できたタイのすべての{name} — {count}件。Google、Reddit、Pantip、Naver、YouTube、ドメイン履歴で照合。有料掲載なし。",
    id: "Setiap {name} di Thailand yang bisa kami verifikasi — {count} tempat, diperiksa silang dengan Google, Reddit, Pantip, Naver, YouTube, dan riwayat domain.",
    vi: "Mọi {name} tại Thái Lan chúng tôi xác minh được — {count} địa điểm, đối chiếu với Google, Reddit, Pantip, Naver, YouTube và lịch sử tên miền.",
  },
  coll_hub_h1: {
    en: "Wellness collections",
    ko: "웰니스 컬렉션",
    th: "คอลเลกชันเวลเนส",
    zh: "康养专题",
    ja: "ウェルネス・コレクション",
    id: "Koleksi wellness",
    vi: "Bộ sưu tập wellness",
  },
  coll_hub_sub: {
    en: "The searches our seven categories hide. Sauna sits inside spa; pilates inside yoga. Here they get their own page — every venue independently verified, none of it paid for.",
    ko: "7개 카테고리에 묻혀 있던 검색어들. 사우나는 스파 안에, 필라테스는 요가 안에 있었습니다. 이제 각자 페이지를 가집니다 — 모두 독립 검증, 광고 없음.",
    th: "การค้นหาที่หมวดหมู่ทั้งเจ็ดของเราซ่อนไว้ ซาวน่าอยู่ในสปา พิลาทิสอยู่ในโยคะ ตอนนี้แต่ละอย่างมีหน้าของตัวเอง",
    zh: "被我们七大分类掩盖的搜索需求。桑拿藏在水疗里，普拉提藏在瑜伽里。现在它们各有专页 — 全部独立核实，无付费。",
    ja: "7つのカテゴリーに埋もれていた検索。サウナはスパの中、ピラティスはヨガの中にありました。今はそれぞれに専用ページがあります。",
    id: "Pencarian yang tersembunyi di tujuh kategori kami. Sauna ada di dalam spa; pilates di dalam yoga. Kini masing-masing punya halaman sendiri.",
    vi: "Những tìm kiếm bị bảy danh mục của chúng tôi che khuất. Sauna nằm trong spa; pilates nằm trong yoga. Giờ mỗi thứ có trang riêng.",
  },
  coll_count: {
    en: "{count} verified", ko: "검증 {count}곳", th: "ตรวจสอบแล้ว {count}", zh: "{count} 家已核实",
    ja: "検証済み{count}件", id: "{count} terverifikasi", vi: "{count} đã xác minh",
  },
  coll_all_collections: {
    en: "All collections", ko: "모든 컬렉션", th: "คอลเลกชันทั้งหมด", zh: "全部专题",
    ja: "すべてのコレクション", id: "Semua koleksi", vi: "Tất cả bộ sưu tập",
  },
  coll_verify_cta: {
    en: "Saw one on Instagram or TikTok? Check the handle →",
    ko: "인스타·틱톡에서 봤나요? 아이디로 확인하기 →",
    th: "เห็นบน Instagram หรือ TikTok? ตรวจสอบชื่อผู้ใช้ →",
    zh: "在 Instagram 或 TikTok 上看到过？用账号核实 →",
    ja: "InstagramやTikTokで見かけた？IDで確認 →",
    id: "Lihat di Instagram atau TikTok? Cek handle-nya →",
    vi: "Thấy trên Instagram hoặc TikTok? Kiểm tra handle →",
  },
  ...
  verify_flag_suspect: {
    en: "Near-perfect rating on very few reviews, and almost nothing else confirms it",
    ko: "리뷰가 매우 적은데 별점만 거의 만점 — 뒷받침하는 다른 근거가 거의 없음",
    th: "คะแนนเกือบเต็มจากรีวิวน้อยมาก และแทบไม่มีหลักฐานอื่นยืนยัน",
    zh: "评论极少却近乎满分，且几乎没有其他佐证",
    ja: "レビューが極端に少ないのに評価がほぼ満点で、裏付けもほとんどありません",
    id: "Rating nyaris sempurna dari sangat sedikit ulasan, dan hampir tidak ada bukti lain",
    vi: "Điểm gần tuyệt đối với rất ít đánh giá, và hầu như không có bằng chứng nào khác",
  },
  ...
  hp_full_guide: { en: "full guide", ko: "전체 가이드", th: "คู่มือฉบับเต็ม", zh: "完整指南", ja: "完全ガイド", ar: "الدليل الكامل" },
  ...
  // ── /[lang]/thailand-in/[month]/ seasonal pages
  season_meta_title: { en: "Thailand in {month}: What to Book & Where to Go", ko: "{month}의 태국: 무엇을 예약하고 어디로 갈까", th: "ประเทศไทยในเดือน{month}: จองอะไรดี ไปที่ไหนดี", zh: "{month}的泰国：该订什么、去哪里", ja: "{month}のタイ：何を予約すべきか、どこへ行くべきか", ar: "تايلاند في {month}: ماذا تحجز وأين تذهب" },
  season_crumb: { en: "Thailand in {month}", ko: "{month}의 태국", th: "ไทยในเดือน{month}", zh: "{month}的泰国", ja: "{month}のタイ", ar: "تايلاند في {month}" },
  season_picks_title: { en: "Top picks for {month}", ko: "{month} 추천 베스트", th: "แนะนำที่สุดสำหรับเดือน{month}", zh: "{month}热门推荐", ja: "{month}のおすすめ", ar: "أفضل الاختيارات لشهر {month}" },
  season_faq_good_q: { en: "Is {month} a good time to visit Thailand?", ko: "{month}은 태국 여행하기 좋은 시기인가요?", th: "เดือน{month}เหมาะกับการไปเที่ยวไทยไหม?", zh: "{month}适合去泰国旅游吗？", ja: "{month}はタイ旅行に良い時期ですか？", ar: "هل {month} وقت جيد لزيارة تايلاند؟" },
  season_faq_book_q: { en: "What should I book in Thailand in {month}?", ko: "{month}에 태국에서 뭘 예약해야 하나요?", th: "ควรจองอะไรในไทยช่วงเดือน{month}?", zh: "{month}在泰国应该预订什么？", ja: "{month}にタイで何を予約すべきですか？", ar: "ماذا يجب أن أحجز في تايلاند في {month}؟" },
  season_faq_book_a: { en: "Based on the season, {niches} are the best fit for {month} — see our verified, trust-ranked listings below.", ko: "이 시즌엔 {niches}가 {month}에 가장 잘 맞습니다 — 아래 검증된 신뢰도순 목록을 확인하세요.", th: "ตามฤดูกาล {niches} เหมาะกับเดือน{month}มากที่สุด — ดูรายชื่อที่ตรวจสอบแล้วด้านล่าง", zh: "根据季节，{niches}最适合{month} — 请查看下方经过验证、按信任度排名的列表。", ja: "この季節は{niches}が{month}に最適です — 下記の検証済み信頼度ランキングをご覧ください。", ar: "بناءً على الموسم، {niches} هي الأنسب لشهر {month} — راجع قوائمنا الموثقة أدناه." },
  season_faq_where_q: { en: "Where in Thailand is best in {month}?", ko: "{month}에 태국 어디가 좋은가요?", th: "เดือน{month}ที่ไหนในไทยดีที่สุด?", zh: "{month}泰国哪里最好？", ja: "{month}のタイではどこが一番良いですか？", ar: "أين في تايلاند الأفضل في {month}؟" },
  season_faq_where_a: { en: "{cities} are the strongest matches for {month}'s weather and events — each has its own city guide with verified listings.", ko: "{month}의 날씨와 이벤트에 가장 잘 맞는 곳은 {cities}입니다 — 각 도시별 검증된 가이드를 확인하세요.", th: "{cities} เหมาะกับสภาพอากาศและกิจกรรมในเดือน{month}มากที่สุด — แต่ละเมืองมีคู่มือพร้อมรายชื่อที่ตรวจสอบแล้ว", zh: "{cities}是最符合{month}天气和活动的地方 — 每个城市都有经过验证的城市指南。", ja: "{month}の天候とイベントに最も適しているのは{cities}です — 各都市に検証済みリストのガイドがあります。", ar: "{cities} هي الأنسب لطقس وفعاليات {month} — لكل مدينة دليلها الخاص بقوائم موثقة." },
  season_other_months_title: { en: "Browse other months", ko: "다른 달 보기", th: "ดูเดือนอื่น", zh: "浏览其他月份", ja: "他の月を見る", ar: "تصفح أشهر أخرى" },
...
// Strings CategoryClient ("use client") needs, pre-resolved server-side by
// its parent page. CategoryClient imports only the *type* below (erased at
// compile time) — never `t()` itself — so this file's ui_i18n.json import
// doesn't end up in the category-page client bundle.
export type CategoryStrings = {
  ...
  pricePremium: string;
  priceLuxury: string;
  filterOutViral: string;
  placesCount: string;
  reset: string;
  noMatches: string;
  tryRemoveFilters: string;
  loadMore: string;
};

export function resolveCategoryStrings(lang: Lang): CategoryStrings {
  return {
    searchPh: t("search_ph", lang),
    sortBy: t("sort_by", lang),
    sortTrust: t("sort_trust", lang),
    sortReviews: t("sort_reviews", lang),
    sortRating: t("sort_rating", lang),
    allLabel: t("all_label", lang),
    filterEstablished: t("filter_established", lang),
    filterActive: t("filter_active", lang),
    filterKorean: t("filter_korean_friendly", lang),
    filterBeginner: t("filter_beginner", lang),
    filter24h: t("filter_24h", lang),
    priceBudget: t("price_budget", lang),
    priceMid: t("price_mid", lang),
    pricePremium: t("price_premium", lang),
    priceLuxury: t("price_luxury", lang),
    filterOutViral: t("filter_out_viral", lang),
    placesCount: t("places_count", lang),
    reset: t("reset", lang),
...
export function resolveVerifyStrings(lang: Lang, placeCount: number): VerifyStrings {
  return {
    ...
    flag: {
      ...
      suspect: t("verify_flag_suspect", lang),
