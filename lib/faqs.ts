...
// Machine-translated corpus, generated by scripts/translate_faqs.mjs and stored
// out of source (regenerable, reviewable as data). Shape:
//   { [slug]: { [lang]: { question, shortAnswer, longAnswer } } }
// Loaded once at module init (server-only — faqs.ts is imported by pages and
// the sitemap, never a client bundle). Absent file → empty map.
type FaqI18n = Record<string, Partial<Record<Lang, { question?: string; shortAnswer?: string; longAnswer?: string }>>>;
const FAQ_I18N: FaqI18n = (() => {
  try {
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), "public/data/faq_i18n.json"), "utf-8")) as FaqI18n;
  } catch {
    return {};
  }
})();
...
// English fallback for a field (bare string = English; object → its `en`).
function englishOf(v: Localized): string {
  if (typeof v === "string") return v;
  return v.en ?? Object.values(v)[0] ?? "";
}

// Resolve a field to one language. Precedence: hand-curated inline translation
// (an object with that lang key) > machine-translated sidecar > English.
function pick(inline: Localized, sidecar: string | undefined, lang: Lang): string {
  if (lang === "en") return englishOf(inline);
  if (typeof inline !== "string" && inline[lang]) return inline[lang] as string;
  if (sidecar) return sidecar;
  return englishOf(inline);
}

export function localizeFaq(faq: Faq, lang: Lang): ResolvedFaq {
  const tr = FAQ_I18N[faq.slug]?.[lang];
  return {
    slug: faq.slug,
    question: pick(faq.question, tr?.question, lang),
    shortAnswer: pick(faq.shortAnswer, tr?.shortAnswer, lang),
    longAnswer: pick(faq.longAnswer, tr?.longAnswer, lang),
    topic: faq.topic,
    related: faq.related,
  };
}
...
// A field carries `lang` when either the inline object has it (curated) or the
// machine-translated sidecar supplies it.
function fieldHasLang(inline: Localized, sidecar: string | undefined, lang: Lang): boolean {
  if (lang === "en") return true;
  if (typeof inline !== "string" && inline[lang]) return true;
  return Boolean(sidecar);
}

// True when this FAQ is genuinely authored in `lang` (en always qualifies),
// counting both inline curated translations and the machine-translated sidecar.
...
export function isFaqTranslated(faq: Faq, lang: Lang): boolean {
  ...
  const tr = FAQ_I18N[faq.slug]?.[lang];
  return (
    fieldHasLang(faq.question, tr?.question, lang) &&
    fieldHasLang(faq.shortAnswer, tr?.shortAnswer, lang) &&
    fieldHasLang(faq.longAnswer, tr?.longAnswer, lang)
  );
}
